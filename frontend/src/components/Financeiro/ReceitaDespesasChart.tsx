...
export function ReceitaDespesasChart({
  ...
  return (
    <div className="flex-1 min-h-70 bg-white dark:bg-[#1E293B] rounded-xl border border-[#E2E8F0] dark:border-[#334155] p-4 sm:p-5 flex flex-col gap-4 min-w-0 transition-colors duration-200">
      ...
      {/* Barras */}
      <div className="flex items-end justify-between gap-1 overflow-x-auto pb-1 sm:gap-2 flex-1">
        {isLoading || !data
          ...
          : data.map((d, idx) => {
              ...
              return (
                 <div key={d.month} className="group flex min-w-10 flex-col items-center gap-1 flex-1 relative">
                  {/* Tooltip */}
      ...
      {data && data[activeIdx] && (
        <div className="flex flex-col gap-2 pt-2 border-t border-[#F1F5F9] dark:border-[#334155] sm:flex-row sm:items-center sm:justify-between">
